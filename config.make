...
ifeq ($(FPZIP_HOME),)
    $(warning WARNING: FPZIP_HOME not specified)
endif

ifeq ($(NETCDF_HOME),)
    $(warning WARNING: NETCDF_HOME not specified)
endif
...
ifeq ($(PWD_BASE),src)
    ...
    H5Z_FPZIP_BASE := ../src
else ifeq ($(PWD_BASE),H5Z-FPZIP)
    H5Z_FPZIP_BASE := ./src
endif
H5Z_FPZIP_PLUGIN := $(H5Z_FPZIP_BASE)/plugin
H5Z_FPZIP_VERSINFO := $(shell grep '^\#define H5Z_FILTER_FPZIP_VERSION_[MP]' $(H5Z_FPZIP_BASE)/H5Zfpzip.h | cut -d' ' -f3 | tr '\n' '.' | cut -d'.' -f-3 2>/dev/null)
...
ifeq ($(wildcard $(FPZIP_HOME)/include),)
FPZIP_INC = $(FPZIP_HOME)/inc
else
FPZIP_INC = $(FPZIP_HOME)/include
endif
FPZIP_LIB = $(FPZIP_HOME)/lib

NETCDF_INC = $(NETCDF_HOME)/include
NETCDF_LIB = $(NETCDF_HOME)/lib
...
MAKEVARS = FPZIP_HOME=$(FPZIP_HOME) HDF5_HOME=$(HDF5_HOME) PREFIX=$(PREFIX)

#.SUFFIXES:
#.SUFFIXES: .c .F90 .h .o .mod

#%.o : %.c
#	$(CC) $< -o $@ -c $(CFLAGS) -I$(H5Z_FPZIP_BASE) -I$(FPZIP_INC) -I$(HDF5_INC)

#%.o %.mod : %.F90
#	$(FC) $< -o $@ -c $(FCFLAGS) -I$(H5Z_FPZIP_BASE) -I$(FPZIP_INC) -I$(HDF5_INC)
