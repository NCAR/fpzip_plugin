...
#define FPZIP_ID 32014
#define FPZIP_LEVEL 32 
...
typedef enum XZIP { NOZIP=0, FPZIP=1} XZIP;
...
#define DEFAULTACTUALDIMS 3
#define DEFAULTDIMSIZE 16 
#define DEFAULTCHUNKSIZE 16 
...
create(XZIP encoder)
{
    ...
    CHECK(nc_def_var(ncid, "var2", NC_FLOAT, actualdims, dimids, &varid2));
    return NC_NOERR;
}
...
{
    unsigned int level = FPZIP_LEVEL;
    unsigned int level2 = 9;
    unsigned int id=0;
    size_t nparams = 0;
    if(encoder == FPZIP) {
        CHECK(nc_def_var_filter(ncid,varid,FPZIP_ID,1,&level));
        CHECK(nc_def_var_filter(ncid,varid2,BZIP2_ID,1,&level2));
	level = 0;
        CHECK(nc_inq_var_filter(ncid,varid,&id,&nparams,&level));
	if(id != FPZIP_ID || nparams != 1 || level != FPZIP_LEVEL)
	    printf("setvarfilter: def/inq mismatch\n");
        level2 = 0;
        CHECK(nc_inq_var_filter(ncid,varid2,&id,&nparams,&level2));
	if(id != BZIP2_ID || nparams != 1 || level2 != 9)
	    printf("setvarfilter: def/inq mismatch\n");
    }
}
...
open(XZIP encoder)
{
	...
	if(nparams > 0) {
        ...
        if(filterid != FPZIP_ID) {
	    printf("Bzip2 id mismatch: %d\n",filterid);
	    return NC_EFILTER;
        }
	if(nparams != 1 && params != NULL && params[0] != FPZIP_LEVEL) {
...
setchunking(void)
{
    ...
    CHECK(nc_def_var_chunking(ncid,varid2,store,chunks));
    if(!verifychunks())
	return NC_EINVAL;
    return NC_NOERR;
}
...
write(void)
{
   ...
   while(odom_more()) {
	...
	CHECK(NC_PUT_VAR1(ncid,varid2,odom,&expected[offset]));
	odom_next();
   }
#else
   stat = NC_PUT_VAR(ncid,varid,expected);
   stat = NC_PUT_VAR(ncid,varid2,expected);
#endif
   return stat;
}
...
{
    int ok = 1;
    unsigned int param = FPZIP_LEVEL;

    printf("\n*** Testing API: fpzip compression.\n");
    reset();

    create(FPZIP);
    setchunking();
    setvarfilter(FPZIP); 
    showparameters(FPZIP);
    ...
    printf("\n*** Testing API: fpzip decompression.\n");
    reset();
    open(FPZIP);
    CHECK(nc_get_var_float(ncid, varid, array));
    ok = compare();
    CHECK(nc_get_var_float(ncid, varid2, array));
    ok = compare();
    CHECK(nc_close(ncid));
    return ok;
}
